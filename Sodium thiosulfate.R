...
data = read.csv("FeCl2 Na2S2O3.csv")
...
## transform independent variable (% oxidation)
...
plot <- ggplot(data, aes(Percent.Oxidation, Log.R))+ 
  ...
  # geom_errorbar(aes(ymin=Log.R-logSE, ymax=Log.R+logSE)) +
  scale_shape_manual(values = c(1, 17)) + 
  labs(x = "Percent Iron Oxidation (sodium thiosulfate inhibited)", y = "Log Inactivation") +
...
plot + geom_line(linetype = 2, data = predicted_MS2, aes(x=Percent.Oxidation, y=MS2_pred)) +
  ...
  labs(x = "Percent Ferrous Oxidation (sodium thiosulfate)", y = "Log Inactivation") +
  expand_limits(y=0)
