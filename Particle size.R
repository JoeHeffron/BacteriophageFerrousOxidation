...
# limit to 60 min
part_data <- part_data[part_data$time < 61, ]

# interpolate a regular matrix from the data
part_s <- interp(part_data$Dose , part_data$time, part_data$Z.Ave)

ax <- list(title = "")
scene2 <- list(xaxis = list(title = "", rangemode = "tozero"), yaxis = ax, zaxis = list(title = "", rangemode = "tozero"))

plot_ly(y = part_s$x, x = part_s$y, z = ~part_s$z, colors = colorRampPalette(c("red", "blue"))(10) ) %>%
  add_surface() %>%
  layout(scene = scene2)

data <- mutate(data, dosesqrt = sqrt(Dose))
data <- mutate(data, timesqrt = sqrt(time))
